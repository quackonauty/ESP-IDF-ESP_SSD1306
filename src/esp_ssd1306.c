...
{
    if (i2c_ssd1306_config->i2c_scl_speed_hz > I2C_ESP_SSD1306_MAX_SCL_SPEED_HZ || i2c_ssd1306_config->width > ESP_SSD1306_MAX_WIDTH || i2c_ssd1306_config->height % 8 != 0 || i2c_ssd1306_config->height > ESP_SSD1306_MAX_HEIGHT)
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Invalid SSD1306 configuration, 'i2c_scl_speed_hz' must be less than or equal to %d, 'width' must be less than or equal to %d, 'height' must be less than or equal to %d and multiple of 8", I2C_ESP_SSD1306_MAX_SCL_SPEED_HZ, ESP_SSD1306_MAX_WIDTH, ESP_SSD1306_MAX_HEIGHT);
        return ESP_ERR_INVALID_ARG;
    }

    ESP_LOGI(ESP_SSD1306_TAG, "Initializing I2C SSD1306...");

    esp_err_t ret = i2c_master_probe(i2c_master_bus, i2c_ssd1306_config->i2c_device_address, I2C_ESP_SSD1306_TIMEOUT_MS / portTICK_PERIOD_MS);
    if (ret != ESP_OK)
    {
        switch (ret)
        {
        case ESP_ERR_NOT_FOUND:
            ESP_LOGE(ESP_SSD1306_TAG, "I2C SSD1306 device not found in address 0x%02X", i2c_ssd1306_config->i2c_device_address);
            break;
        case ESP_ERR_TIMEOUT:
            ESP_LOGE(ESP_SSD1306_TAG, "I2C SSD1306 device timeout in address 0x%02X", i2c_ssd1306_config->i2c_device_address);
            break;
        default:
            ESP_LOGE(ESP_SSD1306_TAG, "I2C SSD1306 device error in address 0x%02X", i2c_ssd1306_config->i2c_device_address);
            break;
        }
    ...
    if (ret != ESP_OK)
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Failed to add I2C SSD1306 device");
        return ret;
    }
    ...
    ret = i2c_master_transmit(i2c_ssd1306->i2c_master_dev, ssd1306_init_cmd, sizeof(ssd1306_init_cmd), I2C_ESP_SSD1306_TIMEOUT_MS / portTICK_PERIOD_MS);
    if (ret != ESP_OK)
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Failed to initialize I2C SSD1306 device");
        return ret;
    }
    ...
    memset(i2c_ssd1306->buffer, 0x00, sizeof(i2c_ssd1306->buffer));

    ESP_LOGI(ESP_SSD1306_TAG, "I2C SSD1306 initialized successfully");
...
{
    ESP_LOGI(ESP_SSD1306_TAG, "Deinitializing I2C SSD1306...");

    esp_err_t ret = i2c_master_bus_rm_device(i2c_ssd1306->i2c_master_dev);
    if (ret != ESP_OK)
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Failed to remove I2C SSD1306 device");
        return ret;
    }

    ESP_LOGI(ESP_SSD1306_TAG, "I2C SSD1306 deinitialized successfully");
...
{
    uint8_t fill_value = fill ? 0xFF : 0x00;
    memset(i2c_ssd1306->buffer, fill_value, i2c_ssd1306->width * i2c_ssd1306->total_pages);
...
esp_err_t i2c_ssd1306_buffer_fill_pixel(i2c_ssd1306_handle_t *i2c_ssd1306, uint8_t x, uint8_t y, bool fill)
{
    ...
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Invalid pixel coordinates, 'x' must be between 0 and %d, 'y' must be between 0 and %d", i2c_ssd1306->width - 1, i2c_ssd1306->height - 1);
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t i2c_ssd1306_buffer_fill_space(i2c_ssd1306_handle_t *i2c_ssd1306, uint8_t x1, uint8_t x2, uint8_t y1, uint8_t y2, bool fill)
{
    ...
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Invalid space coordinates, 'x1' and 'x2' must be between 0 and %d, 'y1' and 'y2' must be between 0 and %d, 'x1' must be less than 'x2', 'y1' must be less than 'y2'", i2c_ssd1306->width - 1, i2c_ssd1306->height - 1);
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t i2c_ssd1306_buffer_text(i2c_ssd1306_handle_t *i2c_ssd1306, uint8_t x, uint8_t y, const char *text, bool invert)
{
    ...
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Invalid text or coordinates: x=%d (max %d), y=%d (max %d)", x, i2c_ssd1306->width - 1, y, i2c_ssd1306->height - 1);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (len > max_chars)
    {
        ESP_LOGW(ESP_SSD1306_TAG, "Text truncated: text columns exceed display width, lost %d columns", x + (len * 8) - i2c_ssd1306->width);
    }

    if (offset != 0 && !has_next_page)
    {
        ESP_LOGW(ESP_SSD1306_TAG, "Vertical truncation: text exceeds display height, lost %d rows", offset);
    }
...
esp_err_t i2c_ssd1306_buffer_image(i2c_ssd1306_handle_t *i2c_ssd1306, uint8_t x, uint8_t y, const uint8_t *image, uint8_t img_width, uint8_t img_height, bool invert)
{
    ...
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Invalid image or coordinates: x=%d (max %d), y=%d (max %d)", x, i2c_ssd1306->width - 1, y, i2c_ssd1306->height - 1);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    {
        ESP_LOGW(ESP_SSD1306_TAG, "Vertical truncation: Lost %d rows", img_height - draw_height);
    }
    if (draw_width < img_width)
    {
        ESP_LOGW(ESP_SSD1306_TAG, "Horizontal truncation: Lost %d columns", img_width - draw_width);
    }
    ...
    for (uint8_t col = 0; col < draw_width; col++)
    {
        for (uint8_t page = 0; page < draw_pages; page++)
        {
            ...
            uint16_t buffer_index = target_page * i2c_ssd1306->width + (x + col);
            if (vertical_offset == 0)
            {
                i2c_ssd1306->buffer[buffer_index] |= img_byte;
            }
            else
            {
                ...
                i2c_ssd1306->buffer[buffer_index] |= lower;
                if (target_page + 1 < num_pages)
                {
                    i2c_ssd1306->buffer[buffer_index + i2c_ssd1306->width] |= upper;
                }
            }
        }
    }
...
esp_err_t i2c_ssd1306_segment_to_ram(i2c_ssd1306_handle_t *i2c_ssd1306, uint8_t page, uint8_t segment)
{
    ...
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Invalid page or segment number, 'page' must be between 0 and %d, 'segment' must be between 0 and %d", i2c_ssd1306->total_pages - 1, i2c_ssd1306->width - 1);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    esp_err_t err = i2c_master_transmit(i2c_ssd1306->i2c_master_dev, ram_addr_cmd, sizeof(ram_addr_cmd), I2C_ESP_SSD1306_TIMEOUT_MS / portTICK_PERIOD_MS);
    if (err != ESP_OK)
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Failed to address the segment to the RAM of the SSD1306 device");
        return err;
    }

    uint16_t index = page * i2c_ssd1306->width + segment;
    uint8_t ram_data_cmd[] = {
        OLED_CONTROL_BYTE_DATA,
        i2c_ssd1306->buffer[index]};
    err = i2c_master_transmit(i2c_ssd1306->i2c_master_dev, ram_data_cmd, sizeof(ram_data_cmd), I2C_ESP_SSD1306_TIMEOUT_MS / portTICK_PERIOD_MS);
    if (err != ESP_OK)
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Failed to transfer the segment to the RAM of the SSD1306 device");
        return err;
    }
...
esp_err_t i2c_ssd1306_segments_to_ram(i2c_ssd1306_handle_t *i2c_ssd1306, uint8_t page, uint8_t initial_segment, uint8_t final_segment)
{
    ...
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Invalid page or segment range, 'page' must be between 0 and %d, 'initial_segment' and 'final_segment' must be between 0 and %d, 'initial_segment' must be less than or equal to 'final_segment'", i2c_ssd1306->total_pages - 1, i2c_ssd1306->width - 1);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Failed to address the initial segment to the RAM of the SSD1306 device");
        return err;
    }

    uint8_t segment_count = final_segment - initial_segment + 1;
    uint8_t ram_data_cmd[segment_count + 1];
    ram_data_cmd[0] = OLED_CONTROL_BYTE_DATA;
    memcpy(&ram_data_cmd[1], &i2c_ssd1306->buffer[page * i2c_ssd1306->width + initial_segment], segment_count);

    err = i2c_master_transmit(i2c_ssd1306->i2c_master_dev, ram_data_cmd, sizeof(ram_data_cmd), I2C_ESP_SSD1306_TIMEOUT_MS / portTICK_PERIOD_MS);
    if (err != ESP_OK)
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Failed to transfer the segments to the RAM of the SSD1306 device");
        return err;
    }
...
esp_err_t i2c_ssd1306_page_to_ram(i2c_ssd1306_handle_t *i2c_ssd1306, uint8_t page)
{
    ...
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Invalid page number, must be between 0 and %d", i2c_ssd1306->total_pages - 1);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Failed to address the page to the RAM of the SSD1306 device");
        return err;
    }

    uint8_t ram_data_cmd[i2c_ssd1306->width + 1];
    ram_data_cmd[0] = OLED_CONTROL_BYTE_DATA;
    memcpy(&ram_data_cmd[1], &i2c_ssd1306->buffer[page * i2c_ssd1306->width], i2c_ssd1306->width);
    err = i2c_master_transmit(i2c_ssd1306->i2c_master_dev, ram_data_cmd, sizeof(ram_data_cmd), I2C_ESP_SSD1306_TIMEOUT_MS / portTICK_PERIOD_MS);
    if (err != ESP_OK)
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Failed to transfer the page to the RAM of the SSD1306 device");
        return err;
    }
...
esp_err_t i2c_ssd1306_pages_to_ram(i2c_ssd1306_handle_t *i2c_ssd1306, uint8_t initial_page, uint8_t final_page)
{
    ...
    {
        ESP_LOGE(ESP_SSD1306_TAG, "Invalid page range, 'initial_page' and 'final_page' must be between 0 and %d, 'initial_page' must be less than or equal to 'final_page'", i2c_ssd1306->total_pages - 1);
        return ESP_ERR_INVALID_ARG;
    }
